          ...
          <tbody>
            {courses.map(course => (
              <tr key={course.id}>
                ...
                <td className="font-bold text-amber-600">₹{course.price || 1499}</td>
                <td className="text-gray-400"><del>₹{course.originalPrice || 2999}</del></td>
                <td>
                  <div className="table-actions">
                    <button className="px-2.5 py-1 text-xs bg-amber-500 hover:bg-amber-600 text-white font-semibold rounded" onClick={() => handleEditCoursePrice(course)} title="Edit Price & Discount">
                      Edit Price
                    </button>
                    <button className="action-btn delete ml-2" onClick={() => handleDeleteCourse(course.id)} title="Delete">
                      <Trash2 size={18} />
                    </button>
                  </div>
                </td>
            ...
            {courses.length === 0 && <tr><td colSpan="6" className="text-center text-muted py-4">No courses found.</td></tr>}
          </tbody>
        </table>
      </div>
    </div>
          ...
          <tbody>
            {packages.map(pkg => (
              <tr key={pkg.id}>
                ...
                <td>
                  <div className="table-actions">
                    <button className="px-2.5 py-1 text-xs bg-amber-500 hover:bg-amber-600 text-white font-semibold rounded mr-2" onClick={() => handleEditPackagePrice(pkg)} title="Edit Package Price & Commission">
                      Edit Price
                    </button>
